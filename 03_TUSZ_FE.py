...
def drop_bad_feature_columns(
    X,
    feature_names,
    y=None,
    max_frac_nonfinite=0.01,
    min_std=1e-6,
    max_frac_near_zero=0.995,
    near_zero_eps=1e-8,
    verbose=True,
):
    """
    Prune noisy / unstable feature columns using:
      1) Name-based rules (very limited: drop subject-delta and graph_*).
      2) Numeric rules:
         - too many NaN/Inf
         - extremely low variance
         - almost-all near-zero values
    ...
    y : array-like or None
        Optional, reserved for future class-conditional rules.
    max_frac_nonfinite : float
        Drop columns where > this fraction of entries are NaN or Inf.
    min_std : float
        Drop columns where std < min_std (on finite entries).
    max_frac_near_zero : float
        Drop columns where > this fraction of entries are "near zero"
        AND the column also has tiny std (i.e., dead feature).
    near_zero_eps : float
        Threshold for "near zero".
    verbose : bool
        Print summary and some example names of dropped features.

    Returns
    -------
    X_keep : np.ndarray
        Filtered feature matrix.
    feature_names_keep : list[str]
        Filtered feature name list.
    keep_mask : np.ndarray[bool]
        Boolean mask applied along the feature axis.
    """
    ...
    # -------------------------------
    # 1) NAME-BASED FILTERING (minimal)
    # -------------------------------
    drop_name_based_idx = []
    ...
    for j, name in enumerate(feature_names):
        ...
        # (a) Drop subject-wise delta for now (leaks subject identity)
        if nm.endswith("_subj_delta"):
            keep_mask[j] = False
            drop_name_based_idx.append(j)
            continue

        # (b) Drop graph_* features: known to be ill-defined / degenerate
        #     on TEST split (std=0, constructed from invalid graphs).
        if nm.startswith("graph_"):
            keep_mask[j] = False
            drop_name_based_idx.append(j)
            continue

        # NOTE: We are *not* dropping TE / GC / PDC / SOC-like features
        # by name anymore. They are allowed to stay unless they fail
        # the numeric rules below. This avoids "hunch-based" dropping.

        # If later we decide to kill specific families by name, you can
        # re-enable patterns like these (kept here as commented templates):

        # # Drop transfer entropy features
        # if "_te_" in nm or nm.endswith("_te") or nm.endswith("_te_mean") or nm.endswith("_te_max"):
        #     keep_mask[j] = False
        #     drop_name_based_idx.append(j)
        #     continue
        #
        # # Drop Granger / PDC-like directional features
        # if "_gc_" in nm or nm.endswith("_gc_mean") or nm.endswith("_gc_max") or "pdc_like" in nm:
        #     keep_mask[j] = False
        #     drop_name_based_idx.append(j)
        #     continue
        #
        # # Example SOC-ish features to drop, if needed:
        # if "psd_slope_2_40hz" in nm:
        #     keep_mask[j] = False
        #     drop_name_based_idx.append(j)
        #     continue
        # if nm.startswith("gt_delta_theta_top3_"):
        #     keep_mask[j] = False
        #     drop_name_based_idx.append(j)
        #     continue
    ...
    for j in range(n_features):
        ...
        frac_nonfinite = 1.0 - finite_mask.mean()

        # (a) Too many NaN/Inf -> drop
        if frac_nonfinite > max_frac_nonfinite:
            keep_mask[j] = False
            drop_nonfinite_idx.append(j)
            continue
        ...
        # (b) Almost-constant -> drop
        if std < min_std:
            keep_mask[j] = False
            drop_lowvar_idx.append(j)
            continue

        # (c) Almost-all near-zero AND very low std -> drop as dead
        frac_near_zero = np.mean(np.abs(col_clean) < near_zero_eps)
        if frac_near_zero > max_frac_near_zero and std < 10 * min_std:
            keep_mask[j] = False
            drop_dead_idx.append(j)
            continue

    # Final cleanup: replace any residual NaN/Inf in kept columns with 0
    X_keep = X[:, keep_mask]
    X_keep = np.nan_to_num(X_keep, copy=False, nan=0.0, posinf=0.0, neginf=0.0)
    feature_names_keep = [fn for fn, k in zip(feature_names, keep_mask) if k]

    if verbose:
        print(f"[Prune] Original features: {n_features}")
        print(f"[Prune] Kept features   : {X_keep.shape[1]}")
        print(f"[Prune] Dropped by name : {len(drop_name_based_idx)}")
        print(f"[Prune] Dropped nonfinite-heavy : {len(drop_nonfinite_idx)}")
        print(f"[Prune] Dropped low-variance    : {len(drop_lowvar_idx)}")
        print(f"[Prune] Dropped dead/nearly-zero: {len(drop_dead_idx)}")

        # Print a few example names for sanity
        def _sample_names(idx_list, label, max_show=5):
            if not idx_list:
                return
            print(f"[Prune] Examples {label}:")
            for k in idx_list[:max_show]:
                print("    ", k, "→", feature_names[k])

        _sample_names(drop_name_based_idx, "name-based drops")
        _sample_names(drop_nonfinite_idx, "nonfinite-heavy drops")
        _sample_names(drop_lowvar_idx, "low-var drops")
        _sample_names(drop_dead_idx, "dead/near-zero drops")

    return X_keep, feature_names_keep, keep_mask
...
def powerlaw_slope_from_psd(x, sfreq, fmin=2.0, fmax=40.0):
    """
    Estimate log–log PSD slope (1/f^beta) between fmin and fmax.
    Returns 0.0 if we can't get a stable estimate.
    """
    x = np.asarray(x, dtype=float)
    if x.size < 4:
        return 0.0
    ...
    logf = np.log(f[mask])
    logP = np.log(Pxx[mask])

    if logf.size < 2:
        return 0.0

    try:
        a, b = np.polyfit(logf, logP, 1)  # logP ≈ a*logf + b
        return float(a)  # slope (usually negative)
    except Exception:
        return 0.0


def powerlaw_linearity_from_psd(x, sfreq, fmin=2.0, fmax=40.0):
    """
    Estimate power-law behavior of the PSD on a log–log scale between fmin and fmax.

    Returns:
      slope        : log–log PSD slope (same as before, usually negative)
      r2           : R^2 of the linear fit on log–log PSD
      resid_std    : std of residuals in log–log space
      resid_mad    : median absolute residual in log–log space
      crit_metric  : |slope| / (1 + resid_std)  -- heuristic SOC/criticality score
    """
    x = np.asarray(x, dtype=float)
    if x.size < 4:
    ...
    try:
        # Linear fit: logP ≈ a*logf + b
        a, b = np.polyfit(logf, logP, 1)
        pred = a * logf + b

        resid = logP - pred
        ss_res = np.sum(resid ** 2)
        ss_tot = np.sum((logP - logP.mean()) ** 2) + 1e-12
        ...
        slope = float(a)
        # Simple “criticality” metric: strong slope + low residuals
        crit_metric = float(abs(slope) / (1.0 + resid_std))

        return slope, r2, resid_std, resid_mad, crit_metric
...
def extract_window_features(X_window, sfreq, channel_names=None,
    ...
    # For cross-band coalition overlap (delta vs theta) in temporal lobe
    temp_topnodes_by_band = {}
    ...
    #     + power-law SOC / criticality metrics from log–log PSD
    # ------------------------------------------------------------------
    plaw_r2_all = []
    plaw_crit_all = []
    plaw_resid_std_all = []
    ...
    for ch in range(n_ch):
        ...
        # classic nonlinear features
        feats[base + 'approx_entropy'] = approximate_entropy(x)
        feats[base + 'higuchi'] = higuchi_fd(x)

        # power-law slope + linearity / residual structure on 2–40 Hz
        slope, r2, resid_std, resid_mad, crit = powerlaw_linearity_from_psd(
            x, sfreq, fmin=2.0, fmax=40.0
        )

        feats[base + 'psd_slope_2_40Hz'] = safe_scalar(slope, 0.0)
        feats[base + 'psd_plaw_r2_2_40Hz'] = safe_scalar(r2, 0.0)
        feats[base + 'psd_plaw_resid_std_2_40Hz'] = safe_scalar(resid_std, 0.0)
        feats[base + 'psd_plaw_resid_mad_2_40Hz'] = safe_scalar(resid_mad, 0.0)
        feats[base + 'psd_plaw_crit_metric_2_40Hz'] = safe_scalar(crit, 0.0)

        plaw_r2_all.append(r2)
        plaw_crit_all.append(crit)
        plaw_resid_std_all.append(resid_std)

    # Global SOC-style summaries across channels (window-level)
    if n_ch > 0:
        plaw_r2_arr = np.asarray(plaw_r2_all, dtype=float)
        plaw_crit_arr = np.asarray(plaw_crit_all, dtype=float)
        plaw_resid_std_arr = np.asarray(plaw_resid_std_all, dtype=float)

        feats["broad_plaw_r2_mean_2_40Hz"] = safe_scalar(
            np.nanmean(plaw_r2_arr), 0.0
        )
        feats["broad_plaw_r2_std_2_40Hz"] = safe_scalar(
            np.nanstd(plaw_r2_arr), 0.0
        )

        feats["broad_plaw_crit_mean_2_40Hz"] = safe_scalar(
            np.nanmean(plaw_crit_arr), 0.0
        )
        feats["broad_plaw_crit_std_2_40Hz"] = safe_scalar(
            np.nanstd(plaw_crit_arr), 0.0
        )

        feats["broad_plaw_resid_std_mean_2_40Hz"] = safe_scalar(
            np.nanmean(plaw_resid_std_arr), 0.0
        )
        feats["broad_plaw_resid_std_std_2_40Hz"] = safe_scalar(
            np.nanstd(plaw_resid_std_arr), 0.0
        )
    ...
    if compute_pairwise:
        for bname, band in BANDS.items():
            ...
            if temporal_idx is not None and temporal_idx.size >= 3:
                ...
                # Lateral (LT vs RT) payoff/leader asymmetry in temporal chain
                lr_feats = temporal_lr_summary(
                    adj_temp,
                    temporal_idx=temporal_idx,
                    channel_names=channel_names,
                    prefix=f"{bname}_temp_"
                )
                for k_lr, v_lr in lr_feats.items():
                    feats[k_lr] = safe_scalar(v_lr, 0.0)

                # --- Store top-3 temporal leaders per band for cross-band overlap ---
                payoff_temp = compute_payoff_vector(adj_temp)  # (n_temp,)
                total_payoff = payoff_temp.sum()
                if total_payoff > 0 and payoff_temp.size >= 1:
                    k = min(3, payoff_temp.size)
                    top_k_idx = np.argsort(payoff_temp)[::-1][:k]
                    temp_topnodes_by_band[bname] = top_k_idx
    ...
    # ------------------------------------------------------------------
    # 3b) Cross-band coalition overlap: delta vs theta leaders (temporal only)
    # ------------------------------------------------------------------
    # We use the top-3 temporal nodes (by payoff) for each band and
    # measure overlap count + Jaccard index.
    if "delta" in temp_topnodes_by_band and "theta" in temp_topnodes_by_band:
        s_delta = set(temp_topnodes_by_band["delta"].tolist())
        s_theta = set(temp_topnodes_by_band["theta"].tolist())
        inter = len(s_delta & s_theta)
        union = len(s_delta | s_theta) if (s_delta or s_theta) else 1

        feats["gt_delta_theta_top3_overlap_count"] = float(inter)
        feats["gt_delta_theta_top3_jaccard"] = float(inter / union)
    else:
        # If we don't have temporal leaders for both bands, define them as 0
        feats["gt_delta_theta_top3_overlap_count"] = 0.0
        feats["gt_delta_theta_top3_jaccard"] = 0.0
...
def get_gt_feature_indices(feature_names):
    """
    Identify indices of GT / connectivity-like features based on name patterns.
    Used both for subjectwise normalization and subjectwise deltas.
    """
    gt_idx = []
    for j, name in enumerate(feature_names):
        if (
            ...
            gt_idx.append(j)
    return np.array(gt_idx, dtype=int)
...
def subjectwise_normalize_gt_features(X, feature_names, meta_list, rec_idx):
    ...
    # Detect GT-like features
    gt_idx = get_gt_feature_indices(feature_names)
    if gt_idx.size == 0:
    ...
    for sid in np.unique(subj_ids):
        ...
        # proper in-place indexing
        block = X_norm[np.ix_(mask, gt_idx)]  # view of the GT columns for this subject

        # just in case anything weird crept in
        block_clean = np.nan_to_num(block, nan=0.0, posinf=0.0, neginf=0.0)

        mu = block_clean.mean(axis=0)
        sigma = block_clean.std(axis=0)
        sigma[sigma < 1e-6] = 1e-6

        X_norm[np.ix_(mask, gt_idx)] = (block_clean - mu) / sigma

    return X_norm


def add_subject_delta_gt_features(X, feature_names, meta_list, rec_idx, y):
    """
    Add subject-wise delta features for GT-like columns.

    For each subject:
      - compute a baseline vector of GT features from that subject's *background* windows (y==0);
        if no background for that subject, fall back to all windows.
      - for each window, delta = X_gt - baseline_subject_gt
    New columns are appended with suffix "_subj_delta".
    """
    X = np.asarray(X, dtype=float)
    N, F = X.shape
    if N == 0 or F == 0:
    ...
    gt_idx = get_gt_feature_indices(feature_names)
    if gt_idx.size == 0:
        # nothing GT-like; nothing to add
        return X, feature_names
    ...
    if y.shape[0] != N:
        raise ValueError(
            f"add_subject_delta_gt_features: y length {y.shape[0]} "
            f"does not match X rows {N}"
        )

    delta_block = np.zeros((N, gt_idx.size), dtype=float)

    for sid in np.unique(subj_ids):
        mask_subj = (subj_ids == sid)
        if not np.any(mask_subj):
            continue

        # prefer background windows for baseline
        mask_bg = mask_subj & (y == 0)
        if not np.any(mask_bg):
            mask_bg = mask_subj  # fall back to all windows for that subject

        block_bg = X[mask_bg][:, gt_idx]
        baseline = block_bg.mean(axis=0)  # [n_gt_features]

        # delta for this subject
        delta_block[mask_subj, :] = X[mask_subj][:, gt_idx] - baseline

    # append deltas as new features
    new_feature_names = list(feature_names)
    for j in gt_idx:
        new_feature_names.append(feature_names[j] + "_subj_delta")

    X_aug = np.concatenate([X, delta_block], axis=1)
    return X_aug, new_feature_names
...
def plot_shap_cumulative(sv, max_k=None, title="Cumulative SHAP mass", save_path=None):
    sv = np.asarray(sv, dtype=float)
    sv = np.abs(sv)
    total = sv.sum()
    if total <= 0:
    ...
    if save_path is not None:
        plt.savefig(save_path, dpi=300)  # 300 dpi for report-quality figures
        print(f"[SHAP] Saved cumulative mass plot to: {save_path}")
        plt.close()
    else:
        plt.show()

def plot_shap_cumulative_multi(shap_dict, max_k=300, title="Cumulative SHAP mass comparison", save_path=None):
    """
    shap_dict: dict like {"RF": shap_vals_rf, "SVM": shap_vals_svm, "XGB": shap_vals_xgb}
    max_k: maximum K to show on the x-axis
    save_path: optional path; if given, save at 300 dpi
    """
    ...
    if save_path is not None:
        ...
        print(f"[SHAP] Saved multi-model cumulative mass plot to: {save_path}")
        plt.close()
    else:
        plt.show()
...
def load_features_with_meta(base_out_dir, split_name, features_suffix=""):
    """
    Loads normalized features (X), labels (y), and feature names (fnames)
    from features{suffix}.npz for the given split.
    ...
    feat_npz = base_out_dir / split_name / f"features{features_suffix}.npz"
...
def evaluate_model_on_split(
    ...
    evaluate on eval_split_name, using features{suffix}.npz.
    """
    base_out_dir = Path(base_out_dir).expanduser().resolve()

    # ---- Load precomputed features ----
    print(f"[Eval] Loading TRAIN features for split '{train_split_name}' ...")
    X_train, y_train, fnames_train = load_features_with_meta(
        base_out_dir,
        train_split_name,
    ...
    X_eval, y_eval, fnames_eval = load_features_with_meta(
        base_out_dir,
        eval_split_name,
        features_suffix=features_suffix,
    )
    ...
    # ---- Sanity: feature name ordering must match BEFORE pruning ----
    if fnames_train != fnames_eval:
        raise RuntimeError("Feature name mismatch between train & eval splits.")

    # ---- Apply feature pruning ONCE (train-based), mirror mask to eval ----
    X_train_pruned, fnames_pruned, keep_mask = drop_bad_feature_columns(
    X_train,
        fnames_train,
        max_frac_nonfinite=0.01,
        min_std=1e-6,
    ...
    X_eval_pruned = X_eval[:, keep_mask]

    X_train = X_train_pruned
    X_eval  = X_eval_pruned
    fnames_train = fnames_pruned
    fnames_eval  = fnames_pruned
...
def compute_and_save_features_for_split(
    base_out_dir,
    split_name,
    add_subject_delta=True,
    features_suffix="",
):
    """
    Loads split_windows.npz for given split, extracts features,
    applies subject-wise GT normalization, optionally adds subject-wise
    delta GT features, and saves to features{suffix}.npz.
    """
    base_out_dir = Path(base_out_dir)
    ...
    windows = data["X"]             # (N_win, n_ch, n_samp) in LE montage
    y = data["y"]                      # labels
    ...
    try:
        ...
        sfreq = 250.0
    print(f"[FE] Split '{split_name}': windows {windows.shape}, sfreq={sfreq}")
    ...
    X_raw, fnames = extract_features_from_windows(
        windows,
        sfreq=sfreq,
        channel_names=ch_names_le,  # now interpreted as TCP names
        is_le=False,                # <<< IMPORTANT: we already did LE->TCP earlier
        compute_pairwise=True,
        compute_graph=True,
    ...
    # ---- Optional: Add subject-wise delta GT features ----
    if add_subject_delta:
        X_final, fnames_final = add_subject_delta_gt_features(
            X_norm,
            fnames,
            meta_list,
            rec_idx,
        ...
        print(
            f"[FE] After adding subject-wise delta GT features '{split_name}': "
            f"{X_final.shape} (was {X_norm.shape})"
        )
    else:
        X_final, fnames_final = X_norm, fnames
        print(
            f"[FE] No subject-wise delta GT features added for '{split_name}'. "
            f"Using shape {X_final.shape}"
        )

    # ---- Save to features{suffix}.npz ----
    out_npz = base_out_dir / split_name / f"features{features_suffix}.npz"
    np.savez_compressed(
        out_npz,
        X=X_final,
        X_raw=X_raw,
        y=y,
        fnames=np.array(fnames_final, dtype=object),
        meta=meta_array,
        rec_idx=rec_idx,
    )
...
def plot_learning_curves_for_model(
    ...
    plt.tight_layout()
    plt.savefig(auprc_path, dpi=300)
    plt.close()
...
if __name__ == "__main__":
    ...
    for split in ["train_internal", "dev_internal", "test_internal"]:
        compute_and_save_features_for_split(BASE_OUT_DIR, split_name=split, add_subject_delta=False, features_suffix="_nodelta")
        #compute_and_save_features_for_split(BASE_OUT_DIR, split_name=split, add_subject_delta=True, features_suffix="")